        ...
        if (v && document.getElementById(s[v - 1]).selectedIndex > 0 || !v) {
            if (dsy.Exists(str)) {
                ...
                for (i = 0; i < ar.length; i++) {
                	...
                	if (key != null) {
                		if (key==ar[i]) {
                			document.getElementById(s[v]).selectedIndex = i + 1;
                		}
                	}
                }
            }
        }
        if (++v < s.length) {change(v,null);}
    }
}
function preselect(p_key,key2,key3) {
    var index;
...
var s = ["prov", "city", "area"];
var opt0 = ["省份", "地级市", "市、县级市、县"];
function setup() {
    for (i = 0; i < s.length - 1; i++)
        document.getElementById(s[i]).onchange = new Function("change(" + (i + 1) + ",null);");
    change(0,null);
}
