...
<html>
	<head>
		...
		<!--弹框-->
		<script src="resource/webview/js/layer_mobile/layer.js" type="text/javascript" charset="utf-8"></script>		<link rel="stylesheet" type="text/css" href="resource/webview/css/style.css"/>
	</head>
	<body>
										...
										<c:forEach var="card" items="${hisInfo['cardInfo']['rows']}" varStatus="status">
											<li class="mui-table-view-cell" onclick="getCardDetail('${card['id']}','${hisInfo['info']['name']}','${card['cardName']}','${hisInfo['info']['headImgUrl']}','${card['firstImgUrl']}','${card['creater']}','${card['publicType']}')">
												<div class="clearfix">
													...
													<a href="javascript:void(0);" class="label-del float-right" onclick="doDeleteCard(this,'${card['id']}')">删除</a>
												</div>
												<div class="card-pics">
													<div class="clearfix">
										...
										<c:forEach var="show" items="${hisInfo['showInfo']['rows']}">
											<li class="mui-table-view-cell" onclick="getShowDetail('${show['id']}','${show['type']}','${show['creater']}')">
												<div class="clearfix">
													...
													<a href="javascript:void(0);" class="label-del float-right" onclick="doDeleteShow(this,'${show['id']}')">删除</a>
												</div>
	...
	<script>
		function doDeleteCard(el,cardId){
			...
			layer.open({
			    ...
			    ,btn: ['删除', '取消']
			    ,yes: function(index){
					$.ajax({ url:'api/deleteCard.do',//从该处获取数据
			    		type:'post',
			    		dataType:'json',
			    		data:{'id':cardId},
				   	    success:function(){
		...
		function doDeleteShow(el,showId){
			...
			layer.open({
			    ...
			    ,btn: ['删除', '取消']
			    ,yes: function(index){
					$.ajax({ url:'api/deleteShow.do',//从该处获取数据
			    		type:'post',
			    		dataType:'json',
			    		data:{'id':showId},
				   	    success:function(){
				   	    ...
				   	    success:function(ret){
				   	    	if(ret.result == 1) {
					   	    	...
					   	    	for(var index in data.rows) {
									...
									html += '<a href="javascript:void(0);" class="label-del float-right" onclick="doDeleteCard(this,\'' + card.id + '\')">删除</a>';
									html += '</div>';
				   	    ...
				   	    success:function(ret){
				   	    	if(ret.result == 1) {
					   	    	...
					   	    	for(var index in data.rows) {
									...
									html += '<a href="javascript:void(0);" class="label-del float-right" onclick="doDeleteShow(this,\'' + show.id + '\')">删除</a>';
									html += '</div>';
